    ...
    public function actionReportDonation($start_day = '')
    {
        ...
        try {
            ...
            $total_ordered = Order::find()
                ->andWhere('created_at >= :start')->addParams([':start' => $to_day])
                ->andWhere('created_at <= :end')->addParams([':end' => $end_day])
                ->andWhere(['status'=>Order::STATUS_TP])
                ->sum('total');
            $total_ordered1 = Order::find()
                ->andWhere('created_at >= :start')->addParams([':start' => $to_day])
                ->andWhere('created_at <= :end')->addParams([':end' => $end_day])
                ->andWhere(['status'=>Order::STATUS_TP1])
                ->sum('total');
            ...
            $report->number_ordered1 = $number_order1;
            ...
            $report->total_ordered1 = $total_ordered1;
